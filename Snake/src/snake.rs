...
use std::collections::LinkedList; //allows us to push and pop from either end of the list //bringing in color
...
pub enum Direction {
    //Handles direction of snake and how keyboard interacts with snake
    Up,
    Down,
    Left,
    Right,
...
#[derive(Debug, Clone)]
struct Block {
    //Block size for our snake
    x: i32,
    y: i32,
}

pub struct Snake {
    //Snakes functionality (direction that snake is travelling in, body is a linked list, and adding to tail)
    direction: Direction,
...
impl Snake {
    ...
    pub fn new(x: i32, y: i32) -> Snake {
        //This is how the snake will start out at the beginning of the game
        let mut body: LinkedList<Block> = LinkedList::new(); //Creating the mutable body
        body.push_back(Block {
            //Snake starts at length of three
            x: x + 2, //Third block x + 2, y
            y,
        });
        body.push_back(Block {
            //second block being x + 1, y
            x: x + 1,
            y,
        });
        body.push_back(Block {
            //First block being x and y
            x,
            y,
        });
    ...
    pub fn draw(&self, con: &Context, g: &mut G2d) {
        //Iterate through list and call draw block function
        for block in &self.body {
            //Renders out green snake
    ...
    pub fn move_forward(&mut self, dir: Option<Direction>) {
        ...
        let new_block = match self.direction {
            //match on actual snake direction
            Direction::Up => Block {
                //If we are going in direction up, then we are going to create a new block
    ...
    pub fn next_head(&self, dir: Option<Direction>) -> (i32, i32) {
        //Get head position and match on direction
    ...
    pub fn restore_tail(&mut self) {
        //Create a block based on tail which we will clone
    ...
    pub fn cut_in_half(&mut self) {
        //Cut snake length in half after eating orange
